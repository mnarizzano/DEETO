function [Cylinder EndPlate1 EndPlate2] = Cyl(X,Y,r,n,cyl_color,closed,lines)
%
...
% modificated from by Divahar Jayaraman Cylinder version
...
for i=1:N
    ...
    if closed==1
        hold on
        EndPlate1(i)=fill3(xx1(:,1),xx2(:,1),xx3(:,1),'r','Visible','off');
        EndPlate2(i)=fill3(xx1(:,2),xx2(:,2),xx3(:,2),'r','Visible','off');
    end
    
    % Plotting the cylinder along the X-Direction with required length starting
    % from Origin
    Cylinder(i)=mesh(xx1,xx2,xx3,'Visible','off');
    ...
    if sum(axis_rot)==0 % parallel vectors
        axis_rot=[0 0 1];
    end
    ...
    if angle_X1X2~=0 % Rotation is not needed if required direction is along X
        rotate(Cylinder(i),axis_rot,angle_X1X2,[0 0 0])
        if closed==1
            rotate(EndPlate1(i),axis_rot,angle_X1X2,[0 0 0])
            rotate(EndPlate2(i),axis_rot,angle_X1X2,[0 0 0])
        end
    end
    ...
    % position
    if closed==1
        set(EndPlate1(i),'XData',get(EndPlate1(i),'XData')+X1(1))
        set(EndPlate1(i),'YData',get(EndPlate1(i),'YData')+X1(2))
        set(EndPlate1(i),'ZData',get(EndPlate1(i),'ZData')+X1(3))
        
        set(EndPlate2(i),'XData',get(EndPlate2(i),'XData')+X1(1))
        set(EndPlate2(i),'YData',get(EndPlate2(i),'YData')+X1(2))
        set(EndPlate2(i),'ZData',get(EndPlate2(i),'ZData')+X1(3))
    end
    set(Cylinder(i),'XData',get(Cylinder(i),'XData')+X1(1))
    set(Cylinder(i),'YData',get(Cylinder(i),'YData')+X1(2))
    set(Cylinder(i),'ZData',get(Cylinder(i),'ZData')+X1(3))
    
    % Setting the color to the cylinder and the end plates
    set(Cylinder(i),'FaceColor',cyl_color)
    if closed==1
        set([EndPlate1(i) EndPlate2(i)],'FaceColor',cyl_color,'Visible','on')
    else
        EndPlate1(i)=[];
        EndPlate2(i)=[];
    end
   set(Cylinder(i),'Visible','on');
    
    % If lines are not needed making it disapear
    if lines==0
        set(Cylinder(i),'EdgeAlpha',0)
    end
end